...
describe("Authentication Plugin", function()
  ...
  describe("OAuth2 Authorization", function()
    ...
    describe("Client Credentials", function()
      ...
      it("should return fail when setting access token and no provision_key", function()
        local response, status = http_client.post(PROXY_SSL_URL.."/oauth2/token", { client_id = "clientid123", client_secret="secret123", scope = "email", grant_type = "client_credentials", access_token = "abc" }, {host = "oauth2_4.com"})
        local body = cjson.decode(response)
        assert.are.equal(400, status)
        assert.are.equal(2, utils.table_size(body))
        assert.are.equal("invalid_provision_key", body.error)
      ...
      it("should return fail when setting access token and invalid provision_key", function()
        local response, status = http_client.post(PROXY_SSL_URL.."/oauth2/token", { client_id = "clientid123", client_secret="secret123", scope = "email", grant_type = "client_credentials", access_token = "abc", provision_key = "hello" }, {host = "oauth2_4.com"})
        local body = cjson.decode(response)
        assert.are.equal(400, status)
        assert.are.equal(2, utils.table_size(body))
        assert.are.equal("invalid_provision_key", body.error)
      ...
      it("should return success with access token and valid provision key", function()
        local response, status = http_client.post(PROXY_SSL_URL.."/oauth2/token", { client_id = "clientid123", client_secret="secret123", scope = "email", grant_type = "client_credentials", authenticated_userid = "hello", provision_key = "provision123", access_token = "abc" }, {host = "oauth2_4.com"})
        local body = cjson.decode(response)
        assert.are.equal(200, status)
        assert.are.equals(3, utils.table_size(body))
        assert.falsy(body.refresh_token)
        assert.are.equals("abc", body.access_token)
        assert.are.equal("bearer", body.token_type)
        assert.are.equal(5, body.expires_in)
      end)
    ...
    describe("Password Grant", function()
      ...
      it("should return supplied access and refresh tokens", function()
        local response, status = http_client.post(PROXY_SSL_URL.."/oauth2/token", { provision_key = "provision123", authenticated_userid = "id123", client_id = "clientid123", client_secret="secret123", scope = "email", grant_type = "password", access_token = "abc", refresh_token = "def" }, {host = "oauth2_5.com"})
        local body = cjson.decode(response)
        assert.are.equal(200, status)
        assert.are.equals(4, utils.table_size(body))
        assert.are.equal("def", body.refresh_token)
        assert.are.equal("abc", body.access_token)
        assert.are.equal("bearer", body.token_type)
        assert.are.equal(5, body.expires_in)
      end)  
  ...
  describe("Refresh Token", function()
    ...
    it("should update tokens with valid refresh_token and provision_key", function()
      local token = provision_token()
      local response, status = http_client.post(PROXY_SSL_URL.."/oauth2/token", { provision_key = "provision123", refresh_token = token.refresh_token, access_token = "abc", new_refresh_token = "def", client_id = "clientid123", client_secret = "secret123", grant_type = "refresh_token" }, {host = "oauth2.com"})
      local body = cjson.decode(response)
      assert.are.equal(200, status)
      assert.are.equals(4, utils.table_size(body))
      assert.are.equal("abc", body.access_token)
    ...
    it("should fail updating tokens with valid refresh_token and invalid provision_key", function()
      local token = provision_token()
      local response, status = http_client.post(PROXY_SSL_URL.."/oauth2/token", { provision_key = "hello", refresh_token = token.refresh_token, access_token = "abc", new_refresh_token = "def", client_id = "clientid123", client_secret = "secret123", grant_type = "refresh_token" }, {host = "oauth2.com"})
      local body = cjson.decode(response)
      assert.are.equal(400, status)
      assert.are.equal(2, utils.table_size(body))
      assert.are.equal("invalid_provision_key", body.error)
